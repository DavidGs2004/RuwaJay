...
export default function LoginPage() {
  ...
  const startReset = () => { setMode('reset'); setResetStep(1); setPassword(''); setCode(''); setResetToken(''); notify(''); };
  const submitReset = async (event) => {
    event.preventDefault(); setBusy(true); notify('');
    try {
      if (resetStep === 1) { await requestPasswordReset(email); setResetStep(2); notify('Revisa tu correo. El código caduca en 10 minutos.'); }
      else if (resetStep === 2) { const data = await verifyResetCode(email, code); setResetToken(data.reset_token); setResetStep(3); notify('Código verificado. Crea tu nueva contraseña.'); }
      else { if (!strongPassword(password)) throw new Error('Completa todos los requisitos de una contraseña fuerte.'); await resetPassword(resetToken, password); setMode('login'); setPassword(''); notify('Contraseña actualizada. Ya puedes iniciar sesión.'); }
  ...
  return <main className="relative flex min-h-[calc(100dvh-72px)] items-start justify-center overflow-hidden bg-[#FAF5EE] px-3 py-6 sm:items-center sm:px-4 sm:py-10 md:min-h-[calc(100dvh-162px)]">
    ...
    <div className="relative z-10 w-full max-w-[500px] overflow-hidden rounded-[24px] border border-[#E8D9C8] bg-white shadow-[0_20px_60px_rgba(45,24,16,0.12)] sm:rounded-[32px]">
      ...
      <div className="p-5 min-[380px]:p-7 sm:p-10">
        ...
        <div className="mb-7 text-center"><RuwaJayLogo size={58} showText className="mb-3 justify-center" /><p className="mb-1 text-xs font-black uppercase tracking-[.18em] text-dorado">¡Qué gusto tenerte aquí!</p><h1 className="text-2xl font-black tracking-tight text-cafe sm:text-3xl">{mode === 'login' ? 'Bienvenido a RuwaJay' : mode === 'register' ? 'Crea tu cuenta' : 'Cambia tu contraseña'}</h1><p className="mt-1.5 text-xs font-semibold text-text-secondary sm:text-sm">{mode === 'reset' ? 'Recupera tu acceso en tres pasos seguros' : 'Tu próximo hogar puede estar más cerca de lo que imaginas'}</p></div>
        ...
        {mode === 'reset' && <div className="mb-6"><div className="flex items-center justify-between">{[1,2,3].map((step, index) => <div key={step} className="flex flex-1 items-center last:flex-none"><div className={`flex h-9 w-9 items-center justify-center rounded-full text-sm font-black ${step <= resetStep ? 'bg-forest text-white' : 'bg-[#E8D9C8] text-text-muted'}`}>{step < resetStep ? <Check size={17}/> : step}</div>{index < 2 && <div className={`mx-2 h-1 flex-1 rounded ${step < resetStep ? 'bg-forest' : 'bg-[#E8D9C8]'}`} />}</div>)}</div><div className="mt-2 grid grid-cols-3 text-center text-[10px] font-bold text-text-muted"><span>Correo</span><span>Código</span><span>Nueva clave</span></div></div>}
        ...
        </form> : <form key={resetStep} onSubmit={submitReset} className="auth-panel-in space-y-4">
          {resetStep === 1 && <EmailField value={email} onChange={setEmail}/>} 
          {resetStep === 2 && <div><label className="mb-2 block text-xs font-extrabold uppercase tracking-wider text-cafe">Código de 6 dígitos</label><div className="flex items-center gap-3 rounded-2xl border border-[#E8D9C8] bg-[#FDFBF7] px-4 py-3.5 focus-within:border-forest"><KeyRound size={18} className="text-forest/70"/><input required inputMode="numeric" pattern="[0-9]{6}" maxLength={6} value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, ''))} placeholder="000000" className="min-w-0 flex-1 bg-transparent text-center text-xl font-black tracking-[.4em] text-cafe outline-none"/></div><p className="mt-2 text-center text-[11px] text-text-muted">Enviado a {email}</p></div>}
          {resetStep === 3 && <PasswordField label="Nueva contraseña" value={password} onChange={setPassword} autoComplete="new-password"/>}
          <button type="submit" disabled={busy} className="flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-forest px-4 py-3 text-sm font-black text-white disabled:opacity-60">{busy ? 'Procesando...' : resetStep === 1 ? 'Enviar código' : resetStep === 2 ? 'Verificar código' : 'Cambiar contraseña'}<ArrowRight size={18}/></button>
          <button type="button" onClick={() => { setMode('login'); notify(''); }} className="w-full text-xs font-extrabold text-text-muted hover:text-cafe">Volver a iniciar sesión</button>
        </form>}
